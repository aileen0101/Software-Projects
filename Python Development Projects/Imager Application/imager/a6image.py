...
class Image(object):
    ...
    def setWidth(self,value):
        ...
        pixel_amount = len(self._data)
        assert value >=0 and pixel_amount % value == 0, repr(value) + 'does not evenly divide the number of pixels'
        ...
        if pixel_amount == 0:
            ...
            self._width = value
            self._height = int(len(self._data) / value)
    ...
    def setHeight(self,value):
        ...
        if pixel_amount == 0:
            ...
            self._height = value
            self._width = int(len(self._data) / value)
    ...
    def __init__(self, data, width):
        ...
        """
        assert _is_pixel_list(data), 'data is not a pixel list'
        assert type(width) == int, 'width is not an integer'
        assert width > 0, 'width is not greater than zero'
        assert len(data)%width ==0, 'width does not evenly divide length'
        ...
        self._width = width
        self._height = int(len(self._data)/width)
    ...
    def __getitem__(self, pos):
        ...
        """
        assert type(pos) == int and 0 <= pos <= (len(self._data) - 1), repr(pos) + 'is not a valid position in the pixel list'
    ...
    def getPixel(self, row, col):
        ...
        assert type(row) == int and row>=0, repr(row) + 'is not an integer greater than or equal to 0'
        assert row < self._height, repr(row) + 'is not less than height'
        assert type(col) == int and col >=0, repr(col) + 'is not an integer greater than or equal to 0'
        assert col < self._width, repr(col) + 'is not less than width'

        #converted position from 2d to 1d:
        position = int(row*self._width + col)

        pixel_value = self._data[position]
        return pixel_value
    ...
    def setPixel(self, row, col, pixel):
        ...
        assert type(col) == int and col >=0, repr(col) + 'is not an integer greater than or equal to 0'
        assert col < self._width, repr(col) + 'is not less than width'
        assert _is_pixel(pixel), repr(pixel) + 'is not a valid pixel'
        
        pix = pixel
        position = int(row*self._width + col)
        #print(type(position))
        self._data[position] = pix
    ...
    def __str__(self):
        ...
        beg_part = self._data[0:int(self._width)]
        row_combine = '[' + str(beg_part)
        #print(row_combine, 'row_combine')
        #row_combine = []
        for row in range(self._height-1):
            ...
            for col in range(self._width):
                new_pixel = self.getPixel(row +1, col) #RGB pixel
            ...
            combined = string + liststr #combine all columns of the row
            #print(combined, 'combined')
            row_combine = row_combine + ','+combined #concatenate with other rows
            #print(row_combine, 'row_combine')
